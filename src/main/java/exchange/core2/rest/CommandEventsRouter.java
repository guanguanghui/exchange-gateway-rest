...
//        resp.json(response).done();
        ...
        for (MatcherTradeEvent evt : matcherTradeEvents) {
            ...
            if (evt.eventType == MatcherEventType.TRADE) {
                ...
                final GatewayUserProfile takerProfile = gatewayState.getOrCreateUserProfile(evt.matchedOrderUid);
                ...
                ticks.add(new TickRecord(tradePrice, evt.size, System.currentTimeMillis()));

            } else if (evt.eventType == MatcherEventType.REJECT) {
                final GatewayUserProfile profile = gatewayState.getOrCreateUserProfile(evt.matchedOrderUid);
                profile.rejectOrder(evt.matchedOrderId, order -> sendOrderUpdate(evt.matchedOrderUid, order));

            } else if (evt.eventType == MatcherEventType.REDUCE) {
                final GatewayUserProfile profile = gatewayState.getOrCreateUserProfile(evt.matchedOrderUid);
                profile.cancelOrder(evt.matchedOrderId, order -> sendOrderUpdate(evt.matchedOrderUid, order));
            }
        }
    ...
    private OrderUpdateEvent handleOrderCommand(OrderCommand cmd) {
        ...
        cmd.processMatcherEvents(evt -> {
            if (evt.eventType == MatcherEventType.TRADE) {
                ...
                MatchingRole role = evt.matchedOrderId == cmd.orderId ? MatchingRole.TAKER : MatchingRole.MAKER;
